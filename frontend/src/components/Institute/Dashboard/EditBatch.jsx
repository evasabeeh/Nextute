...
const EditBatch = () => {
  ...
  // Welcome back popup
  useEffect(() => {
    const hasVisited = localStorage.getItem("hasVisitedEditBatch");
    if (hasVisited) {
      toast(
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Edit your batch details seamlessly.
            </p>
          </div>
        </div>,
        {
    ...
    localStorage.setItem("hasVisitedEditBatch", "true");
  }, []);
    ...
    if (instituteDashboardData && hasRenderedOnce) {
      try {
        ...
        const batch = batchData.find(
          (b, index) => (b.id || `batch-${index}`) === id
        );
        if (batch) {
          setFormData({
            name: batch.name || `Batch ${batchData.indexOf(batch) + 1}`,
            category: batch.category || "",
            details: batch.details || "",
            feeRange: batch.feeRange || "",
            medium: batch.medium || "",
          });
        } else {
